...
public class CryptoJuliusCaesarApp {
    ...
    public static String encrypt(String s, int key){
        StringBuilder encrypted = new StringBuilder();  //φτιαχνω strings (βαζει char το ένα μετά το άλλο)
        char ch;
        for (int i = 0 ; i < s.length(); i++){
            ch = s.charAt(i);
            if (Character.isUpperCase(ch)) encrypted.append(cipher(ch,key)); // Με το apend προσθέτω στο string builder.
